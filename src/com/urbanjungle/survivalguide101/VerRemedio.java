...
public class VerRemedio extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// Here we declare the bookmark button, that we're going to use for
		// adding or deleting bookmarks
		final ImageView bookmark = (ImageView) findViewById(R.id.imgBookmark);
		bookmark.setClickable(true);

		final Remedio actual = Remedio.getRemedio(cabecera);
		...
		regresar.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				Intent a = new Intent(VerRemedio.this, Home.class);
	...
	public void addBookmark(Remedio actual) {
		...
		if (count < 25) {
			Bookmark nuevo = new Bookmark(id, actual.id, 1);
			nuevo.save();
		} else {
			AlertDialog.Builder msg = new AlertDialog.Builder(VerRemedio.this);
	...
	public void deleteBookmark(Remedio actual) {
		Bookmark viejo = Bookmark.getBookmark(actual.id, 1);
		viejo.delete();

	}

	public boolean existe(Remedio actual) {
		try {
			Bookmark nuevo = Bookmark.getBookmark(actual.id, 1);
